...
<template>
  <div class="relative w-full" ref="selectRef">
    ...
    <div 
      @click="toggle"
      :class="[
        'w-full bg-slate-100/60 border-2 rounded-2xl p-4 text-sm font-bold flex items-center justify-between transition-all cursor-pointer',
        isOpen ? 'bg-white border-blue-400 shadow-xl shadow-blue-100 ring-4 ring-blue-500/5' : 'border-slate-100/10',
    ...
    <Transition name="pop">
      <div 
        v-if="isOpen" 
        class="absolute z-[999] top-full left-0 right-0 mt-3 bg-white border-2 border-slate-50 rounded-[1.8rem] shadow-2xl shadow-blue-900/10 overflow-hidden py-3 animate-pop origin-top"
      >
        <div class="max-h-[250px] overflow-y-auto scrollbar-soft">
          <div 
            ...
            @click="select(opt.id)"
            :class="[
              'px-6 py-3.5 text-sm font-bold transition-all cursor-pointer flex items-center justify-between group',
              modelValue == opt.id ? 'bg-blue-50 text-blue-600' : 'text-slate-500 hover:bg-slate-50 hover:text-blue-500'
            ]"
          >
            <span>{{ opt.name }}</span>
            <SIGAPIcons 
...
/* Animation reuse from global or local */
.animate-pop {
...
.pop-leave-active { animation: pop 0.2s cubic-bezier(0.16, 1, 0.3, 1) reverse; }
</style>
